...
(C-rate, temperature, form factor). The output is `last_observed_SOH +
...
1. **Cell picker**, grouped by dataset (Oxford pouch / VL coin cells).
2. **Predicted vs. actual SOH trajectory** — the headline panel. Pick an
   ...
   RMSE and max error.
3. **SOH & capacity fade overview** — SOH, discharge capacity, mean cycle
   temperature and coulombic efficiency per cycle.
4. **Single live prediction** — runs the selected architecture in-browser
   for one chosen reference cycle.
5. **Cycle detail** — full voltage / current / temperature curves for any
   individual cycle.
6. **Compare all cells** — every cell's fade curve on one axis, coloured by
   dataset.
7. **About** — architecture comparison table and the full per-cell LOCO
