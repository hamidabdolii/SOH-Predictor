...
train.py (models/loco_predictions.json) as each cell's `loco_by_arch` field
— genuine out-of-sample predictions (that cell's data was held out of
training entirely for its own fold), one entry per recurrent architecture
(lstm / gru / rnn / bilstm), which is what the web app's "predicted vs
actual" trajectory chart displays and what its model picker switches
between. Do NOT substitute in-browser predictions from the production
models here: those were trained on every cell including this one, so their
predictions on it would not be a fair out-of-sample test.

`loco` (singular) is retained as an alias for the LSTM entry so older
consumers of this JSON keep working.
...
def export_all(npz_dir: str, out_dir: str, loco_predictions_path: str | None = None):
    ...
    for npz_path in sorted(npz_dir.glob("*.npz")):
        ...
        cell_data = {
            ...
            "curves": {},
            # Out-of-sample leave-one-cell-out predictions for this cell,
            # per architecture (see module docstring). Empty/null when not
            # available — the frontend shows "not available" rather than
            # fabricating a chart in that case.
            "loco_by_arch": {
                arch: preds[cell_id]
                for arch, preds in loco_predictions.items()
                if cell_id in preds
            },
            # Back-compat alias: the LSTM entry under the old singular key.
            "loco": loco_predictions.get("lstm", {}).get(cell_id),
        }
