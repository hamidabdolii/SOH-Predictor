...
function renderFleetStats() {
  ...
  if (locoResults && locoResults.per_cell && locoResults.per_cell.lstm) {
    predictionCount = Object.values(locoResults.per_cell.lstm).reduce((sum, m) => sum + (m.n || 0), 0);
  }

  const lstmMae = locoResults ? locoResults.lstm.mean_mae : null;
  const naiveMae = locoResults ? locoResults.naive.mean_mae : null;
  ...
  el.innerHTML =
    ...
    (lstmMae !== null
      ? statTile("brain", "Mean LOCO MAE (LSTM)", `${lstmMae.toFixed(2)} pts`, "good", "good")
      : "") +
    (naiveMae !== null
...
function renderLocoTable() {
  ...
  catalog.forEach((cell) => {
    ...
    tr.innerHTML = `
      ...
      <td class="num good-text">${lstm.mae.toFixed(2)}</td>
      <td class="num">${lstm.rmse.toFixed(2)}</td>
...
function updateAboutModelText() {
  ...
  const lstmMae = locoResults.lstm.mean_mae;
  const naiveMae = locoResults.naive.mean_mae;
  const rfMae = locoResults.rf.mean_mae;
  const nCells = catalog.length;
  el.innerHTML =
    `A small residual LSTM (16 hidden units) predicts next-cycle SOH from a 10-cycle window of summary features ` +
    `(capacity, coulombic efficiency, step durations, temperature, voltage), conditioned on C-rate/temperature/form-factor. ` +
    `Validated with leave-one-cell-out cross-validation across all ${nCells} cells in the pool (VL coin cells and Oxford pouch cells): ` +
    `mean absolute error of <strong>${lstmMae.toFixed(2)}% SOH</strong> on cells never seen during training &mdash; beating both a naive ` +
    `persistence baseline (${naiveMae.toFixed(2)}%) and a Random Forest baseline (${rfMae.toFixed(2)}%). All inference runs locally in your ` +
    `browser via ONNX Runtime Web; no data leaves your device.`;
}
