...
function renderCellGrid() {
  ...
  catalog.forEach((cell) => {
    ...
    card.innerHTML = `
      ...
      <div class="meta">${cell.n_cycles} cycles tested</div>
      <span class="eol-badge">80% SOH @ cycle ${cell.eol_cycle_80pct > 0 ? cell.eol_cycle_80pct : "n/a"}</span>
    `;
  ...
  sohChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { title: { display: true, text: "SOH (%)" }, min: 0, max: 105 },
      },
...
function renderCapacityChart(data) {
  ...
  capacityChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { title: { display: true, text: "Cycle" } },
        y: { title: { display: true, text: "Capacity (mAh)" }, beginAtZero: true },
      },
...
function renderCEChart(data) {
  ...
  ceChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { title: { display: true, text: "Cycle" } },
        y: { title: { display: true, text: "CE (ratio)" } },
      },
      plugins: { legend: { display: false } },
    },
  });
...
function renderStats(data) {
  ...
  statsRow.innerHTML = `
    <div class="stat"><div class="label">Reference capacity</div><div class="value">${(cRef * 1000).toFixed(2)} mAh</div></div>
    <div class="stat"><div class="label">80% EOL cycle</div><div class="value">${eol > 0 ? eol : "n/a"}</div></div>
    <div class="stat"><div class="label">Final SOH (cycle ${data.cycles[data.cycles.length - 1]})</div><div class="value">${finalSoh.toFixed(1)}%</div></div>
  `;
}
...
function setupTrajectoryPanel(data) {
  ...
  if (!data.loco) {
    btn.disabled = true;
    btn.textContent = "Not available for this cell";
  ...
  btn.disabled = false;
  btn.textContent = "Run full prediction";
...
function updateTrajectoryStats(data, shownCount) {
  ...
  const statsRow = document.getElementById("trajectory-stats");
  statsRow.innerHTML = `
    <div class="stat"><div class="label">Predictions made</div><div class="value accent">${shownCount} / ${data.loco.target_cycle.length}</div></div>
    <div class="stat"><div class="label">Mean absolute error</div><div class="value ${mae < 1.5 ? "good" : "warn"}">${mae.toFixed(2)} pts</div></div>
    <div class="stat"><div class="label">Max error so far</div><div class="value">${maxErr.toFixed(2)} pts</div></div>
  `;
}
...
function renderCurveCharts(data, cycle) {
  ...
  const commonOpts = (yLabel) => ({
    ...
    maintainAspectRatio: false,
    scales: {
      x: { title: { display: true, text: "Normalized time within step" } },
...
function setupPredictPanel() {
  ...
  document.getElementById("predict-btn").addEventListener("click", async () => {
    ...
    try {
      ...
      document.getElementById("predict-result").innerHTML = `
        <div class="result-item"><div class="label">Predicted SOH @ cycle ${cellData.cycles[targetIdx]}</div><div class="value">${pred.toFixed(2)}%</div></div>
        <div class="result-item"><div class="label">Actual measured SOH</div><div class="value">${actual.toFixed(2)}%</div></div>
        <div class="result-item"><div class="label">Absolute error</div><div class="value ${errClass}">${err.toFixed(2)} pts</div></div>
      `;
    } catch (e) {
